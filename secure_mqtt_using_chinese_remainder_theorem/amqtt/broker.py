...
class Broker:
    ...
    #Check if some number is prime or not
    ...
    def generate_random_integer(self, start, end):
        random_int = secrets.randbelow(end - start)
        random_int += start
        return random_int
    ...
    def generate_large_prime(self):
        while True:
            ...
            if self.is_prime(random_integer):
                return random_integer

    def initialize_key_setup(self, private_keys):

        # Step 1: Compute a_g as the product of all private keys k_i
        a_g = np.prod(private_keys)

        # Step 2: Compute x_i for each i
        x = [a_g // ki for ki in private_keys]

        # Step 3: Compute y_i such that x_i * y_i ≡ 1 mod k_i for each i
        y = [mod_inverse(xi, ki) for xi, ki in zip(x, private_keys)]

        # Step 4: Multiply all users x_i and y_i values and store them in the variables var_i
        var = [xi * yi for xi, yi in zip(x, y)]

        # Step 5: Compute the value μ as the sum of all var_i
        mu = sum(var)
    ...
    async def handle_leave(self, topic):
        ...
        if topic in self._subscriptions:
            ...
            #Generate Gk
            gk = self.generate_random_integer(1000000000000000000000000000000000000, self.q - 1)
        ...
        try:
            while a == 1:
                ...
                if (packet_contents[1] == '1'):
                    ...
                    while True:
                        tt = self.generate_random_integer(self.q + 1, self.p - 1)
                        flag = True
                        for ele in self._perma_key_table.values():
                            if (self.gcd(ele, tt) != 1):
                                flag = False
                                break
                    ...
                    # for val in self._perma_key_table.values():
                    #     print(val)
    ...
    def __init__(self, config=None, loop=None, plugin_namespace=None):
        ...
        #q for operations
        self.q = self.generate_random_integer(2, math.ceil(self.p / 4))
