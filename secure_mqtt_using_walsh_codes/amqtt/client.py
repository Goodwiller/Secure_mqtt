...
class MQTTClient:
    ...
    async def get_GK(self, walsh_row_num, walsh_code_curr, walsh_message, perma_key):
        ...
        return await self.decrypt_message(perma_key, bytes.fromhex(hex_num))

    def __init__(self, client_id=None, config=None, is_broker_client = False):
        ...
        #Table to store channel GKS
        self.channel_keys = dict()

        self.walsh_rows = dict()
    ...
    async def connect(
        ...
        try:
            connection_res = await self._do_connect()
            ...
            if not auto_reconnect:
                ...
                connection_res = await self.reconnect()


        if self.is_broker_client == True :
            return connection_res
        else :
            #Subscribing to KEYDIS channel to get perma key
            ...
            #sending request for perma key to KEY DIs client via KEYDIS channel
            req = "||1||" + self.client_id + "||" + (pem) + "||"
            ...
            try:
                while a == 1:
                    ...
                    packet_contents[2] = packet_contents[2].replace("\\\\", "\\")
                    if (packet_contents[1] == '2' and packet_contents[2] == self.client_id):

                        cipher_key = packet_contents[3]
    ...
    async def disconnect(self):
        ...
        if self.session.transitions.is_connected():
            ...
            #Clear channel keys and unsubscribe from all channels
            self.channel_keys = dict()
    ...
    async def wait_for_value(topic):
        while topic not in self.channel_keys:
            await asyncio.sleep(2)  # Wait for a short duration to avoid blocking
        return self.channel_keys[topic]
    ...
    async def publish(self, topic, message, qos=None, retain=None, ack_timeout=None):
        ...
        #Encrypt unicast message for broker here
        msg = message
        msg = str(msg).split('||')

        # # print(msg)
        if len(msg) > 1 and msg[1] in self.subscribed_channels or topic == "KEYDIS":
            message = message

            await self._handler.mqtt_publish(
            ...
            counter = 0
            while True:
                if topic not in self.channel_keys:
                    print("Tried to publish on \"" + topic + "\" but channel key not received yet....")
                    await asyncio.sleep(1)
                elif (counter == 5):
                    print("Tried to publish on \"" + topic + "\" 5 times but channel key not received yet (aborting)....")
                    return
                else: 
                    print("Publishing message on channel \"" + topic + "\"")
                    message = await self.encrypt_data(message, self.channel_keys[topic])
    ...
    async def subscribe(self, topics):
        ...
        """
        #Add channels to subscribed channels table
        for topic in topics:
    ...
    async def unsubscribe(self, topics):
        ...
        # Deleting channel from subscribed list
        for topic in topics:
            if topic[0] in self.subscribed_channels:
                del self.subscribed_channels[topic[0]]
                del self.channel_keys[topic[0]]
    ...
    async def deliver_message(self, timeout=None):
        ...
        if deliver_task in done:
            ...
            if len(message) > 1 and message[1] in self.subscribed_channels:
                ...
                channel = deliver_task.result().publish_packet.variable_header.topic_name
                if channel != "KEYDIS":
